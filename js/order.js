let linkEn =
  'https://privatelabels.wine/dashboard/?rest_route=/wp/v2/order_process&slug=en';
let linkCn =
  'https://privatelabels.wine/dashboard/?rest_route=/wp/v2/order_process&slug=en-copy';
let linkYue =
  'https://privatelabels.wine/dashboard/?rest_route=/wp/v2/order_process&slug=en-copy-3';
...
let orderImg = document.getElementById('labelSVG');
...
function fillOrder(json) {
  ...
  let orderDescJSON = acf.main_description;
  let titleGalleryJSON = acf.examples_gallery.title_of_gallery;

  let examples = Object.entries(acf.examples_gallery.gallery_images); //loopable chunk of json file
  //assigning json content to DOM
  topTitle.innerHTML = topTitleJSON;
  orderDesc.innerHTML = orderDescJSON;
  orderImg.src = acf.order_process_image;
  ...
  examples.forEach(example => {
    ...
    if (example[1]) {
      newImage.src = example[1];
      newSliderItem.setAttribute('data-src', example[1]);
